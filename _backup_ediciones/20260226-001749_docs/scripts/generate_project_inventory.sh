...
script_desc() {
  case "$1" in
    ...
    scripts/install_desktop.sh) echo "Instala integración de escritorio y handler `afirma://` en Linux." ;;
    ...
    scripts/run_web_compat_server.sh) echo "Arranca/para el servidor web-compat local (`cmd/autofirma`) para pruebas con sedes." ;;
    ...
    scripts/test_active_go.sh) echo "Ejecuta tests Go activos (`./cmd/...`, `./pkg/...`) para ciclo rápido de desarrollo." ;;
    scripts/test_websocket_cancel.sh) echo "Test de integración para cancelación WebSocket y retorno `CANCEL` al navegador." ;;
    ...
    scripts/ws_send_afirma_uri.py) echo "Cliente WS para enviar una URI `afirma://...` al servidor local (diagnóstico)." ;;
    ...
    scripts/package_linux_artifact.sh) echo "Genera artefacto Linux empaquetado desde `dist/`/`release`." ;;
    scripts/build_macos_osxcross.sh) echo "Compila para macOS desde Linux con `osxcross`." ;;
    ...
    packaging/linux/make_linux_release.sh) echo "Construye bundle e instalador Linux (`.run`/`.tar.gz`) con binarios, Qt runtime y extensiones." ;;
    packaging/linux/build_and_install.sh) echo "Build+install Linux desde carpeta `packaging/` (wrapper del flujo de despliegue)." ;;
    ...
    packaging/windows/autofirma_windows_installer.nsi) echo "Script NSIS del instalador Windows (secciones, registro `afirma://`, accesos directos)." ;;
...
exe_desc() {
  case "$1" in
    cmd/autofirma) echo "Binario principal Go (`autofirma`): motor de firma, protocolo `afirma://`, WSS, REST local y frontends ligeros Fyne/Gio." ;;
    cmd/browser-bridge) echo "Host de Native Messaging (`autofirma-browser-bridge`) para Chrome/Firefox (stdin/stdout JSON)." ;;
    cmd/gui-qml) echo "GUI de escritorio C++/Qt6/QML (`autofirma-gui-qml` / `autofirma-qt`). Cliente del core vía REST/IPC." ;;
    cmd/gui-widgets) echo "GUI clásica C++/Qt Widgets (`autofirma-gui-widgets`). Cliente del core vía REST." ;;
    ...
    *) echo "Componente ejecutable o fuente principal en `cmd/`." ;;
  esac
}
...

{
  ...
  echo "- Versión (`pkg/version/version.go`): ${version_go:-desconocida}"
  echo "- Versión (`VERSION`): ${version_txt:-desconocida}"
  echo
  echo "## Objetivo"
  echo
  ...
  echo
  echo "## Binarios de salida habituales (`dist/` y releases)"
  echo
  cat <<'EOF'
...
EOF
  echo
  echo "## Scripts del Proyecto (incluye `scripts/` y `packaging/`)"
  echo
