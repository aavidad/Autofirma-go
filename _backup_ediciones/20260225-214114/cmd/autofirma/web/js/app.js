...
function setManualConnectIdleState() {
    ...
    AppState.reconnectAttempts = 0;
    DOM.statusText.textContent = "Pulsa Reconectar";
    DOM.statusIndicator.classList.remove('connected');
    DOM.statusIndicator.style.background = 'var(--muted-color)';
    if (DOM.restStatus) {
        DOM.restStatus.innerHTML = `<i class="fa-solid fa-plug-circle-exclamation"></i> Conexión manual (pulsa Reconectar)`;
        DOM.restStatus.className = 'text-muted';
    }
    if (DOM.tlsStatus) {
        DOM.tlsStatus.innerHTML = `<i class="fa-solid fa-circle-info"></i> Se comprobará al conectar`;
        DOM.tlsStatus.className = 'text-muted';
    }
    syncSignButtonState();
}
