...
async function checkConnection() {
    ...
    try {
        ...
        if (health && health.ok) {
            ...
            loadCertificatesForSign().catch((e) => {
                log('Certificados', `No se pudieron precargar para firma: ${e.message}`, 'warn');
            });
...
DOM.btnValidateFile?.addEventListener('click', async () => {
    ...
    try {
        ...
        const msg = valid
            ...
            : `Firma no válida (${fmt}). ${r.reason || 'Sin detalle.'}`;
        log('Verificación', msg, valid ? 'success' : 'warn');
        alert(msg);
    } catch (e) {
        log('Verificación', `Error al validar: ${e.message}`, 'error');
