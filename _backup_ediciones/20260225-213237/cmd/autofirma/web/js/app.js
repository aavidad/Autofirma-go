...
function scheduleAutoReconnect() {
    ...
    if (AppState.reconnectAttempts >= 6) return;
    AppState.reconnectAttempts += 1;
    const delay = Math.min(500 * AppState.reconnectAttempts, 2000);
    AppState.reconnectTimer = setTimeout(() => {
        AppState.reconnectTimer = null;
        log('Conexión', `Reintento automático (${AppState.reconnectAttempts}/6)...`, 'info');
        checkConnection();
    }, delay);
}
