...
func (s *CoreService) GeneratePdfPreview(pdfPath string, page int) (string, float64, float64, error) {
	if page < 1 {
		page = 1
	}

	W, H := 595.28, 841.89 // A4 default

	// Intentar obtener dimensiones reales con pdfinfo
	if infoCmd := exec.Command("pdfinfo", "-f", fmt.Sprintf("%d", page), "-l", fmt.Sprintf("%d", page), pdfPath); infoCmd != nil {
		if out, err := infoCmd.Output(); err == nil {
			lines := strings.Split(string(out), "\n")
			for _, line := range lines {
				if strings.Contains(line, "Page") && strings.Contains(line, "size:") {
					parts := strings.Fields(line)
					for i, part := range parts {
						if part == "size:" && i+3 < len(parts) {
							w, _ := strconv.ParseFloat(parts[i+1], 64)
							h, _ := strconv.ParseFloat(parts[i+3], 64)
							if w > 0 && h > 0 {
								W, H = w, h
								break
							}
	...
	tmpDir := os.TempDir()
	outPrefix := filepath.Join(tmpDir, fmt.Sprintf("af_preview_%d_%d", os.Getpid(), time.Now().UnixNano()))

	// Intentar pdftocairo
	cmd := exec.Command("pdftocairo", "-png", "-singlefile", "-f", fmt.Sprintf("%d", page), "-l", fmt.Sprintf("%d", page), pdfPath, outPrefix)
	if err := cmd.Run(); err != nil {
		// Fallback a pdftoppm
		cmd = exec.Command("pdftoppm", "-png", "-singlefile", "-f", fmt.Sprintf("%d", page), "-l", fmt.Sprintf("%d", page), pdfPath, outPrefix)
		if err := cmd.Run(); err != nil {
			return "", W, H, fmt.Errorf("no se pudo generar previsualización (pdftocairo/pdftoppm no encontrados)")
		}
	}
	...
	imgData, err := os.ReadFile(outPng)
	if err != nil {
		return "", W, H, fmt.Errorf("error leyendo resultado de previsualización: %v", err)
	}

	return base64.StdEncoding.EncodeToString(imgData), W, H, nil
}
