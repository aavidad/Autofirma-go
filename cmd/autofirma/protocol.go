...
var signingDomainPatternsCache = struct {
...
func allowedSigningDomainPatterns() []string {
	signingDomainPatternsCache.allowedOnce.Do(func() {
		raw := resolveDomainPatternsRaw(
			...
			filepath.Join("config", "dominios_firma_permitidos.txt"),
			filepath.Join("config", "default_allowed_sign_domains.txt"),
			builtinFallbackAllowedSigningDomains,
		)
		signingDomainPatternsCache.allowed = parseDomainPatternList(raw)
	})
	out := make([]string, len(signingDomainPatternsCache.allowed))
	copy(out, signingDomainPatternsCache.allowed)
	return out
}

func autoTrustedSigningDomainPatterns() []string {
	signingDomainPatternsCache.autoOnce.Do(func() {
		raw := resolveDomainPatternsRaw(
			...
			filepath.Join("config", "dominios_firma_autoconfiados.txt"),
			filepath.Join("config", "default_auto_trust_sign_domains.txt"),
			builtinFallbackAutoTrustedSigningDomains,
		)
		signingDomainPatternsCache.auto = parseDomainPatternList(raw)
	})
	out := make([]string, len(signingDomainPatternsCache.auto))
	copy(out, signingDomainPatternsCache.auto)
	return out
}

func resolveDomainPatternsRaw(envCSVPrimary, envCSVCompat, envFilePrimary, envFileCompat, relativeFilePrimary, relativeFileCompat, fallback string) string {
	if raw := strings.TrimSpace(os.Getenv(envCSVPrimary)); raw != "" {
		return raw
	}
	if raw := strings.TrimSpace(os.Getenv(envCSVCompat)); raw != "" {
		return raw
	}
	if fp := strings.TrimSpace(os.Getenv(envFilePrimary)); fp != "" {
		if raw := strings.TrimSpace(readDomainPatternsFile(fp)); raw != "" {
			return raw
		}
	}
	if fp := strings.TrimSpace(os.Getenv(envFileCompat)); fp != "" {
		if raw := strings.TrimSpace(readDomainPatternsFile(fp)); raw != "" {
			return raw
		}
	}
	if exe, err := os.Executable(); err == nil {
		cfgPathPrimary := filepath.Join(filepath.Dir(exe), relativeFilePrimary)
		if raw := strings.TrimSpace(readDomainPatternsFile(cfgPathPrimary)); raw != "" {
			return raw
		}
		cfgPathCompat := filepath.Join(filepath.Dir(exe), relativeFileCompat)
		if raw := strings.TrimSpace(readDomainPatternsFile(cfgPathCompat)); raw != "" {
			return raw
		}
	}
	return fallback
...
func readDomainPatternsFile(path string) string {
...
func parseDomainPatternList(raw string) []string {
...
func isAutoTrustedSigningDomain(host string) bool {
	host = strings.ToLower(strings.TrimSpace(host))
	if host == "" {
		return false
	}
	for _, pattern := range autoTrustedSigningDomainPatterns() {
...
func ensureTrustedSigningDomain(host string) error {
	...
	if isAutoTrustedSigningDomain(host) {
		return nil
	}
...
func importTrustedSigningDomainsFromFile(path string) (added int, processed int, err error) {
	path = strings.TrimSpace(path)
	if path == "" {
		return 0, 0, fmt.Errorf("ruta de fichero vacía")
	}
	data, err := os.ReadFile(path)
	if err != nil {
		return 0, 0, err
	...
	for _, raw := range lines {
		...
		for _, p := range parts {
			host := normalizeSigningDomainInput(p)
			if host == "" {
				continue
			}
			processed++
