...
type UI struct {
	...
	BtnViewOutput   widget.Clickable // View output file
	...
	BtnServerStart         widget.Clickable
	BtnServerStop          widget.Clickable
	ShowAbout              bool
	...
	OutputFile         widget.Editor
	ServerPort         widget.Editor
	ServerToken        widget.Editor
	ServerFingerprints widget.Editor
	ChkServerHTTPS     widget.Bool
...
func NewUI(w *app.Window) *UI {
	...
	ui.OutputFile.SingleLine = true
	ui.OutputFile.Submit = true
	...
	ui.ServerPort.SingleLine = true
	ui.ServerPort.SetText("63118")
	ui.ServerToken.SingleLine = true
	ui.ServerFingerprints.SingleLine = true
	ui.initScriptTests()
...
func (ui *UI) browseFile() {
	...
	if path != "" {
		...
		if ui.Mode == 0 {
			...
			ui.OutputFile.SetText(base + "_firmado" + ext)
		}
		ui.Window.Invalidate()
	}
}

func (ui *UI) browseOutputFile() {
	title := "Seleccionar destino del fichero firmado"
	initialPath := ui.OutputFile.Text()
	if initialPath == "" && ui.InputFile.Text() != "" {
		initialPath = ui.InputFile.Text()
	}

	path, err := pickSavePath(title, initialPath)
	if err != nil {
		ui.StatusMsg = "Error: No se pudo abrir el dialogo de guardado: " + err.Error()
		ui.Window.Invalidate()
		return
	}
					...
					layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						return layout.UniformInset(unit.Dp(16)).Layout(gtx, func(gtx layout.Context) layout.Dimensions {
							return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
								...
								// Output PDF Path (only in sign mode)
								layout.Rigid(func(gtx layout.Context) layout.Dimensions {
									if ui.Mode != 0 {
										return layout.Dimensions{}
									}
									return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
										...
										layout.Rigid(func(gtx layout.Context) layout.Dimensions {
											return material.H6(ui.Theme, "Destino del PDF firmado (opcional):").Layout(gtx)
										}),
										layout.Rigid(func(gtx layout.Context) layout.Dimensions {
											return layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}.Layout(gtx,
												layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
													ed := material.Editor(ui.Theme, &ui.OutputFile, "Ruta de salida (por defecto: _firmado)")
													ed.Editor.Submit = true
													return ed.Layout(gtx)
												}),
												layout.Rigid(func(gtx layout.Context) layout.Dimensions {
													if ui.BtnBrowseOutput.Clicked(gtx) {
														go ui.browseOutputFile()
													}
													browseOut := material.Button(ui.Theme, &ui.BtnBrowseOutput, "Cambiar...")

													// View Output Button
													if ui.BtnViewOutput.Clicked(gtx) {
														path := ui.OutputFile.Text()
														if path != "" {
															go func() { _ = openExternal(path) }()
														}
													}
													viewOutBtn := material.Button(ui.Theme, &ui.BtnViewOutput, "Ver")
													if ui.OutputFile.Text() == "" {
														viewOutBtn.Background = color.NRGBA{R: 200, G: 200, B: 200, A: 255}
													} else {
														viewOutBtn.Background = color.NRGBA{R: 0, G: 150, B: 200, A: 255}
													}

													return layout.Flex{Axis: layout.Horizontal}.Layout(gtx,
														layout.Rigid(func(gtx layout.Context) layout.Dimensions {
															return layout.UniformInset(unit.Dp(8)).Layout(gtx, func(gtx layout.Context) layout.Dimensions {
																return viewOutBtn.Layout(gtx)
															})
														}),
														layout.Rigid(func(gtx layout.Context) layout.Dimensions {
															return layout.UniformInset(unit.Dp(8)).Layout(gtx, func(gtx layout.Context) layout.Dimensions {
																return browseOut.Layout(gtx)
															})
														}),
													)
												}),
									...
									if ui.ShowAbout {
										return layout.UniformInset(unit.Dp(8)).Layout(gtx, func(gtx layout.Context) layout.Dimensions {
											return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
												...
												layout.Rigid(func(gtx layout.Context) layout.Dimensions {
													if !ui.ChkExpertMode.Value {
														msg := material.Caption(ui.Theme, "Activa modo experto para usar el panel de pruebas integradas y servidor REST.")
														msg.Color = color.NRGBA{R: 95, G: 95, B: 95, A: 255}
														return layout.UniformInset(unit.Dp(8)).Layout(gtx, msg.Layout)
													}
													return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
														layout.Rigid(func(gtx layout.Context) layout.Dimensions {
															return ui.layoutServerPanel(gtx)
														}),
														layout.Rigid(func(gtx layout.Context) layout.Dimensions {
...
func pickSavePath(title string, initialPath string) (string, error) {
	if runtime.GOOS == "windows" {
		ext := filepath.Ext(initialPath)
		if ext != "" {
			ext = ext[1:] // remove dot
		}
		if path, canceled, err := nativeSaveFileDialogWindows(title, initialPath, ext); err == nil {
			if canceled || strings.TrimSpace(path) == "" {
				return "", nil
			}
			return strings.TrimSpace(path), nil
		}
		// Fallback to powershell for save dialog
		ps := "$ErrorActionPreference='Stop'; " +
			"Add-Type -AssemblyName System.Windows.Forms; " +
			"$dlg = New-Object System.Windows.Forms.SaveFileDialog; " +
			"$dlg.Title = '" + psQuote(title) + "'; " +
			"$dlg.FileName = '" + psQuote(initialPath) + "'; " +
			"$dlg.Filter = 'PDF files (*.pdf)|*.pdf|All files (*.*)|*.*'; " +
			"if ($dlg.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) { Write-Output $dlg.FileName }"
		cmd := exec.Command("powershell", "-NoProfile", "-STA", "-NonInteractive", "-Command", ps)
		configureGUICommand(cmd)
	...
	// Linux/Unix path pickers (zenity save mode)
	cmd := exec.Command("zenity", "--file-selection", "--save", "--confirm-overwrite", "--title="+title, "--filename="+initialPath)
	configureGUICommand(cmd)
	out, err := cmd.Output()
	if err == nil {
		return strings.TrimSpace(string(out)), nil
	}
	cmd = exec.Command("kdialog", "--getsavefilename", initialPath)
	configureGUICommand(cmd)
	out, err = cmd.Output()
	if err == nil {
		return strings.TrimSpace(string(out)), nil
...
func (ui *UI) layoutServerPanel(gtx layout.Context) layout.Dimensions {
	return layout.UniformInset(unit.Dp(8)).Layout(gtx, func(gtx layout.Context) layout.Dimensions {
		return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return material.H6(ui.Theme, "Servidor API REST / Acceso Remoto").Layout(gtx)
			}),
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return layout.Spacer{Height: unit.Dp(8)}.Layout(gtx)
			}),
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}.Layout(gtx,
					layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						...
						return material.Body2(ui.Theme, "Puerto:").Layout(gtx)
					}),
					layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
						e := material.Editor(ui.Theme, &ui.ServerPort, "63118")
						return e.Layout(gtx)
					}),
				)
			}),
			...
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return layout.Flex{Axis: layout.Horizontal, Alignment: layout.Middle}.Layout(gtx,
					layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						gtx.Constraints.Min.X = gtx.Dp(unit.Dp(60))
						return material.Body2(ui.Theme, "Token:").Layout(gtx)
					}),
					layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
						e := material.Editor(ui.Theme, &ui.ServerToken, "Opcional")
						return e.Layout(gtx)
					}),
				)
			}),
			...
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
					layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						return material.Body2(ui.Theme, "Huellas Digitales (SHA-256 CSV):").Layout(gtx)
					}),
					layout.Rigid(func(gtx layout.Context) layout.Dimensions {
						e := material.Editor(ui.Theme, &ui.ServerFingerprints, "Opcional: h1,h2...")
						return e.Layout(gtx)
					}),
				)
			}),
			...
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return material.CheckBox(ui.Theme, &ui.ChkServerHTTPS, "Habilitar HTTPS (Requiere generar certificados)").Layout(gtx)
			}),
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				return layout.Spacer{Height: unit.Dp(16)}.Layout(gtx)
			}),
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				if ui.IsRestRunning {
					if ui.BtnServerStop.Clicked(gtx) {
						ui.stopRestServer()
					}
					btn := material.Button(ui.Theme, &ui.BtnServerStop, "Detener Servidor")
					btn.Background = color.NRGBA{R: 180, G: 0, B: 0, A: 255}
					return btn.Layout(gtx)
				} else {
					if ui.BtnServerStart.Clicked(gtx) {
						go ui.startRestServer()
					}
					btn := material.Button(ui.Theme, &ui.BtnServerStart, "Iniciar Servidor REST")
					btn.Background = color.NRGBA{R: 0, G: 120, B: 0, A: 255}
					return btn.Layout(gtx)
				}
			}),
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				if ui.RestServerErr != nil {
					lbl := material.Caption(ui.Theme, "Error: "+ui.RestServerErr.Error())
					lbl.Color = color.NRGBA{R: 200, G: 0, B: 0, A: 255}
					return layout.UniformInset(unit.Dp(4)).Layout(gtx, lbl.Layout)
				}
				if ui.IsRestRunning {
					lbl := material.Caption(ui.Theme, "Servidor en ejecución...")
...
func (ui *UI) startRestServer() {
	...
	log.Printf("[UI/Server] Iniciando servidor REST en %s (HTTPS=%t)", addr, useHTTPS)
	ui.RestServerErr = nil
	ui.IsRestRunning = true
	ui.Window.Invalidate()

	// runRESTServer is blocking, so we run it in a goroutine (called via 'go ui.startRestServer()')
	err := runRESTServer(addr, token, 10*time.Minute, fingerprints, useHTTPS)
	if err != nil {
		log.Printf("[UI/Server] Error en servidor REST: %v", err)
		ui.RestServerErr = err
		ui.IsRestRunning = false
	}
	ui.Window.Invalidate()
}

func (ui *UI) stopRestServer() {
	// Currently there is no clean way to stop the http.Server from runRESTServer easily
	// without refactoring runRESTServer to accept a context or returning the server instance.
	// For now, we just indicate it's "stopping" (though it might stay bound until process exit).
	// TODO: Refactor runRESTServer to support graceful shutdown.
	ui.IsRestRunning = false
	ui.StatusMsg = "El servidor REST se detendrá al cerrar la aplicación (pendiente implementación de parada en caliente)."
	ui.Window.Invalidate()
}
