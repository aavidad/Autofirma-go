...
func (ui *FyneUI) buildSignTab() fyne.CanvasObject {
	ui.InputFileEntry = widget.NewEntry()
	ui.InputFileEntry.SetPlaceHolder("Ruta del fichero PDF...")
	ui.InputFileEntry.OnChanged = func(v string) {
		ui.InputFile = v
		// Suggest output path if empty or default-pattern
		if ui.OutputFile == "" || strings.Contains(ui.OutputFile, "_firmado") {
			ext := filepath.Ext(v)
			base := strings.TrimSuffix(v, ext)
			if v != "" {
				ui.OutputFile = base + "_firmado" + ext
				if ui.OutputFileEntry != nil {
					ui.OutputFileEntry.SetText(ui.OutputFile)
				}
			}
		}
	}
	...
	ui.OutputFileEntry.SetPlaceHolder("Ruta de destino (opcional)...")
	ui.OutputFileEntry.OnChanged = func(v string) {
		ui.OutputFile = v
	}

	dropZone := ui.buildDropZone(func(name string) {
		ui.InputFileEntry.SetText(name)
		ui.InputFile = name
	...
	content := []fyne.CanvasObject{
		...
		container.NewPadded(container.NewVBox(
			widget.NewLabelWithStyle("Archivo de entrada:", fyne.TextAlignLeading, fyne.TextStyle{Bold: true}),
			container.NewBorder(nil, nil, nil, container.NewHBox(
				widget.NewButtonWithIcon("Ver", theme.VisibilityIcon(), func() {
					if ui.InputFile != "" {
						_ = openExternal(ui.InputFile)
					}
				}),
				widget.NewButtonWithIcon("Examinar...", theme.SearchIcon(), func() {
					ui.openPDFPicker(func(name string) {
						ui.InputFileEntry.SetText(name)
					})
				}),
			), ui.InputFileEntry),
		...
		container.NewPadded(container.NewVBox(
			widget.NewLabelWithStyle("Destino del PDF firmado (opcional):", fyne.TextAlignLeading, fyne.TextStyle{Bold: true}),
			container.NewBorder(nil, nil, nil, container.NewHBox(
				widget.NewButtonWithIcon("Ver", theme.VisibilityIcon(), func() {
					if ui.OutputFile != "" {
						_ = openExternal(ui.OutputFile)
					}
				}),
				widget.NewButtonWithIcon("Cambiar...", theme.DocumentSaveIcon(), func() {
					defaultPath := ui.OutputFile
					if defaultPath == "" && ui.InputFile != "" {
						ext := filepath.Ext(ui.InputFile)
						defaultPath = strings.TrimSuffix(ui.InputFile, ext) + "_firmado" + ext
					}
					path, canceled, err := protocolSaveDialog(defaultPath, "pdf")
					if !canceled && err == nil && path != "" {
						ui.OutputFileEntry.SetText(path)
					}
				}),
			), ui.OutputFileEntry),
		)),
...
func (ui *FyneUI) buildVerifyTab() fyne.CanvasObject {
	ui.VerifyFileLabel = widget.NewLabel("Ningún PDF seleccionado para verificar")
...
func (ui *FyneUI) signCurrentFileCore() {
	...
	if target == "" { // Only suggest if ui.OutputFile is empty
...
func (ui *FyneUI) runExpertSelectCertDialogFyne() {
	...
	if err != nil {
		ui.SetStatus(withSolution("Error en selección avanzada de certificado: "+err.Error(), "Revisa el almacén de certificados."))
		return
	}
