...
var (
	...
	cliDomainFileFlag        = flag.String("domain-file", "", "Fichero de dominios para importación masiva (domains-import)")
	...
	cliFicheroDominiosFlagCast = flag.String("fichero-dominios", "", "Alias de -domain-file")
)
...
func applyCLISpanishAliases() {
	...
	if v := strings.TrimSpace(*cliFicheroDominiosFlagCast); v != "" {
		*cliDomainFileFlag = v
	}
}
...
func runCLIDomainsImport() error {
	f := strings.TrimSpace(*cliDomainFileFlag)
	if f == "" {
		return fmt.Errorf("debe indicar -domain-file o -fichero-dominios para domains-import")
	}
	added, processed, err := importTrustedSigningDomainsFromFile(f)
	if err != nil {
		return err
	}
	domains := trustedSigningDomainsSnapshot()
	...
	fmt.Printf("Importación completada: procesados=%d añadidos=%d fichero=%s\n", processed, added, f)
	return runCLIDomainsList()
}
...
func appendCLIDetailedUsage(out *strings.Builder) {
	...
	out.WriteString("  -operacion listar-dominios | anadir-dominio -dominio <host> | eliminar-dominio -dominio <host> | importar-dominios -fichero-dominios <ruta>\n")
	out.WriteString("    Gestiona whitelist de dominios de firma confiados.\n")
	out.WriteString("    Compatibilidad: -op domains-list|domains-add|domains-remove|domains-import y -domain-file.\n")
...
func writeCLIHelp(out io.Writer, program string) {
	...
	_, _ = fmt.Fprintf(out, "  %s -modo-cli -operacion anadir-dominio -dominio firma.ejemplo.gob.es\n", program)
	_, _ = fmt.Fprintf(out, "  %s -modo-cli -operacion importar-dominios -fichero-dominios /ruta/dominios.txt\n", program)
