...
var (
	...
	ipcSocketFlag         = flag.String("ipc-socket", "", "Ruta del socket Unix para el modo IPC (por defecto: auto-generada segura)")

	// Gestión de allowlist Native Messaging (host).
	allowlistOpFlag      = flag.String("allowlist-op", "", "Operación de allowlist Native Messaging: list|add|remove|set-require")
	allowlistTypeFlag    = flag.String("allowlist-type", "", "Tipo de ID de allowlist: chromium|firefox")
	allowlistIDFlag      = flag.String("allowlist-id", "", "ID de extensión para operación add/remove")
	allowlistRequireFlag = flag.String("allowlist-require", "", "Valor booleano para set-require: true|false")
	allowlistFileFlag    = flag.String("allowlist-file", "", "Ruta del fichero allowlist JSON (opcional)")

	// Alias en castellano.
	allowlistOpCastFlag      = flag.String("allowlist-operacion", "", "Alias de -allowlist-op")
	allowlistTypeCastFlag    = flag.String("allowlist-tipo", "", "Alias de -allowlist-type")
	allowlistIDCastFlag      = flag.String("allowlist-identificador", "", "Alias de -allowlist-id")
	allowlistRequireCastFlag = flag.String("allowlist-requerir", "", "Alias de -allowlist-require")
	allowlistFileCastFlag    = flag.String("allowlist-archivo", "", "Alias de -allowlist-file")
	restTLSFlag              = flag.Bool("rest-tls", false, "Habilitar cifrado TLS (HTTPS) para el servidor REST")
	restTLSCastFlag          = flag.Bool("rest-https", false, "Alias de -rest-tls")

	importP12Flag      = flag.String("import-p12", "", "Ruta a un archivo .p12 para importar en los almacenes del sistema y navegadores")
	importPasswordFlag = flag.String("import-password", "", "Contraseña del archivo .p12")
...
func applyAllowlistSpanishAliases() {
	if v := strings.TrimSpace(*allowlistOpCastFlag); v != "" {
		*allowlistOpFlag = v
	}
	if v := strings.TrimSpace(*allowlistTypeCastFlag); v != "" {
		*allowlistTypeFlag = v
	}
	if v := strings.TrimSpace(*allowlistIDCastFlag); v != "" {
		*allowlistIDFlag = v
	}
	if v := strings.TrimSpace(*allowlistRequireCastFlag); v != "" {
		*allowlistRequireFlag = v
	}
	if v := strings.TrimSpace(*allowlistFileCastFlag); v != "" {
		*allowlistFileFlag = v
	}
}

func applyImportSpanishAliases() {
	if v := strings.TrimSpace(*importP12CastFlag); v != "" {
		*importP12Flag = v
	}
	if v := strings.TrimSpace(*importPasswordCastFlag); v != "" {
		*importPasswordFlag = v
	}
}
...
func main() {
	...
	applyAllowlistSpanishAliases()
	applyImportSpanishAliases()
	...
	if handled, err := maybeRunAllowlistCLI(
		strings.TrimSpace(*allowlistOpFlag),
		strings.TrimSpace(*allowlistTypeFlag),
		strings.TrimSpace(*allowlistIDFlag),
		strings.TrimSpace(*allowlistRequireFlag),
		strings.TrimSpace(*allowlistFileFlag),
	); handled {
		if err != nil {
			log.Printf("Error gestionando allowlist: %v", err)
			os.Exit(1)
		}
		return
	}
	...
	if *importP12Flag != "" {
		log.Printf("Iniciando importación CLI de certificado: %s", *importP12Flag)
		if err := certstore.ImportP12ToSystem(*importP12Flag, *importPasswordFlag); err != nil {
			log.Printf("ERROR en importación de certificado: %v", err)
			os.Exit(1)
		}
		log.Printf("Importación completada satisfactoriamente.")
		return
	}

	if *ipcSocketFlag == "" {
		*ipcSocketFlag = getSecureIPCSocketPath()
	}

	// Setup logging
	logPath, err := applog.Init("autofirma")
	if err != nil {
	...
	if *restModeFlag {
		...
		if sock != "" {
			...
			if err := runRESTServer(addr, strings.TrimSpace(*restTokenFlag), *restSessionTTLFlag, strings.TrimSpace(*restCertFPFlag), *restTLSFlag); err != nil {
...
func runQtFrontend(protocolArg string) error {
	...
	candidates := executableCandidates(exeDir, "autofirma-gui-qml")
	candidates = append(candidates, executableCandidates(exeDir, "autofirma-gui-widgets")...)
	candidates = append(candidates, "autofirma-gui-qml", "autofirma-gui-widgets")
	...
	if qtBin == "" {
		return fmt.Errorf("frontend Qt (autofirma-gui-qml o autofirma-gui-widgets) no encontrado")
	}
...
func writeSpanishDetailedUsage(out io.Writer, program string) {
	...
	_, _ = fmt.Fprintln(out, "  -allowlist-op list|add|remove|set-require")
	_, _ = fmt.Fprintln(out, "    Gestiona la allowlist de Native Messaging usada por autofirma-host.")
	_, _ = fmt.Fprintln(out, "    Requiere -allowlist-type chromium|firefox y -allowlist-id en add/remove.")
	_, _ = fmt.Fprintln(out, "    En set-require use -allowlist-require true|false.")
	_, _ = fmt.Fprintln(out, "    Alias castellanos: -allowlist-operacion, -allowlist-tipo, -allowlist-identificador, -allowlist-requerir, -allowlist-archivo.")
	...
	_, _ = fmt.Fprintln(out, "  -import-p12 <archivo.p12> [-import-password <password>]")
	_, _ = fmt.Fprintln(out, "    Importa el certificado personal en los almacenes del sistema y navegadores detectados.")
	_, _ = fmt.Fprintln(out, "    Alias castellano: -importar-p12 [-contrasena-p12 <password>].")
	_, _ = fmt.Fprintln(out, "")
...
func runSelectedServerMode(mode string) error {
	...
	case "rest":
		...
		log.Printf("Iniciando en modo servidor REST en %s", addr)
		return runRESTServer(addr, strings.TrimSpace(*restTokenFlag), *restSessionTTLFlag, strings.TrimSpace(*restCertFPFlag), *restTLSFlag)
	case "ipc":
		sock := strings.TrimSpace(*ipcSocketFlag)
		log.Printf("Iniciando motor IPC en modo headless: %s", sock)
	...
	case "ambas":
		...
		go func() {
			...
			log.Printf("Iniciando en modo servidor REST en %s", addr)
			errCh <- runRESTServer(addr, strings.TrimSpace(*restTokenFlag), *restSessionTTLFlag, strings.TrimSpace(*restCertFPFlag), *restTLSFlag)
		}()
...
func getSecureIPCSocketPath() string {
	userName := "default"
	if u, err := os.UserHomeDir(); err == nil {
		userName = filepath.Base(u)
	}
	if runtime.GOOS == "windows" {
		return `\\.\pipe\autofirma_ipc_` + userName
	}
	runtimeDir := os.Getenv("XDG_RUNTIME_DIR")
	if runtimeDir == "" {
		runtimeDir = os.TempDir()
	}
	return filepath.Join(runtimeDir, "autofirma_ipc_"+userName+".sock")
}
