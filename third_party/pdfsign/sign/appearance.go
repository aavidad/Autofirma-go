...
func fitLineToWidth(line string, maxChars int) string {
	if maxChars < 1 {
		return ""
	}
	r := []rune(line)
...
func prepareTextLines(text string) []string {
...
func computeTextLayout(lines []string, rectWidth, rectHeight float64) ([]string, float64, float64, float64) {
	padding := math.Max(2, math.Min(rectWidth, rectHeight)*0.08)
	availableW := rectWidth - 2*padding
	availableH := rectHeight - 2*padding
	...
	longest := 1
	for _, l := range lines {
		if n := runeCount(l); n > longest {
			longest = n
		}
	}

	heightLimit := availableH / (float64(len(lines)) * 1.2)
	widthLimit := availableW / (float64(longest) * 0.5)
	maxByOldBehavior := rectHeight * 0.4 // 50% del tamaño previo (~0.8*alto).
	fontSize := math.Min(maxByOldBehavior, math.Min(heightLimit, widthLimit))
	if fontSize < 3 {
		fontSize = 3
	}

	lineHeight := fontSize * 1.2
	maxLines := int(availableH / lineHeight)
	if maxLines < 1 {
		return nil, 0, 0, 0
	}
	...
	maxChars := int(availableW / (fontSize * 0.5))
	for i := range lines {
		lines[i] = fitLineToWidth(lines[i], maxChars)
	}

	textX := padding
	textY := rectHeight - padding - fontSize // baseline primera línea (alineado arriba).
	return lines, fontSize, textX, textY
}

func drawText(buffer *bytes.Buffer, lines []string, fontSize float64, x, y float64) {
...
func (context *SignContext) createAppearance(rect [4]float64) ([]byte, error) {
	...
	if shouldDisplayText {
		text := strings.TrimSpace(context.SignData.Appearance.Text)
		if text == "" {
